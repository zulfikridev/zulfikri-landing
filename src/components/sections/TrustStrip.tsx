  ...
  return (
    <section className="relative border-y border-white/5 bg-slate-900/30 overflow-hidden">
      ...
      <div className="container-max section-padding py-12">
          ...
          {STATS.map((stat, i) => (
            ...
            >
              <div className="text-3xl md:text-4xl font-bold text-white mb-1">
                ...
                <span className="text-slate-400 text-xl">{stat.suffix}</span>
              </div>
        ...
        >
          <span className="text-xs text-slate-600 uppercase tracking-wider mr-2">
            Industries:
          </span>
          {INDUSTRIES.map((industry) => (
            <span
              key={industry.name}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800/60 border border-slate-700/40 text-sm text-slate-300"
            >
          ...
          <div className="overflow-hidden">
            ...
            >
              {[...techLogos, ...techLogos].map((tech, i) => (
                <span
                  key={`${tech.name}-${i}`}
                  className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-800/40 border border-slate-700/30 text-sm font-mono font-medium ${tech.color}`}
                >
                  {tech.name}
                </span>
              ))}
